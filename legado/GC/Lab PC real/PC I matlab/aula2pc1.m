...
t  = [0:dt:T-dt];
ta = t(2) - t(1); 
x1 = zeros(size(t)); % gera um vetor zeros do tamanho de t
...
% 1500-375: 1500+375 , ou  seja, 750 posicoes de 1
fport=200; %frequencia da portadora
x2 = x1.*sin(2*pi*fport*t);
...
figure, plot(f,10*log10(fftshift(abs(X1))));hold on;
plot(f,10*log10(fftshift(abs(X2))),'k'), grid;
% seno novo
x2=x2.*sin(2*pi*fport/2*t);
%plot(t, x2), hold all;
plot(f,10*log10(fftshift(abs(X2))),'r'), grid, hold off;